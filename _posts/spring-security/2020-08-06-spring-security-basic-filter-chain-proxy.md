...
excerpt: FilterChainProxy가 언제 생성되고, 무엇을 담당하고 있는지 알아 보자.
---
...
`FilterChainProxy`가 언제 생성되고, 무엇을 담당하고 있는지 알아 보자. 
...
- 앞에서 얘기했던 `springSecurityFilterChain` bean은 도대체 어디에 있는지 궁금할 수 있다.
- 앞에서 `DelegatingFilterProxy`까지 생성 되고 난 다음에 `WebSecurityConfiguration`에서 `springSecurityFilterChain` bean을 등록 한다.
